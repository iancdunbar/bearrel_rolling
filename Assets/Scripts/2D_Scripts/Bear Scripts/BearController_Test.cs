...
public class BearController_Test : MonoBehaviour {
	...
	public Vector2 decell;
	public Vector2 accell;
	public Vector2 minSpeed;
	public Vector2 terminalAccel;
	private Vector2 decellValue;
	private Vector2 accellValue;
	public Vector2 invulnSpeed;
	...
	void FixedUpdate () 
    {
		...
		if (rbody.velocity.x >= minSpeed.x || rbody.velocity.y <= minSpeed.y)
		{
			rbody.velocity -= decell;
		}

		currentSpeed = rbody.velocity.magnitude;
		currentVelocity = rbody.velocity;

		invulnSlider.value = rbody.velocity.x;

		if (rbody.velocity.x >= terminalAccel.x){
			bearInvuln = true;
			//rbody.velocity = Vector3.ClampMagnitude (rbody.velocity, max_speed);
			//decell = minSpeed;
			rbody.velocity += invulnSpeed;
			StartCoroutine (TerminalCoolDown());
		}
